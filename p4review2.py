...
class P4Review(object):
    ...
    def pull_data_from_p4(self):
        ...
        if self.cfg.review_counter:
            ...
            for rvw in rv:
                ...
                if rvwers:
                    cl = p4.run_describe(["-s", chgno])[
                        0
                    ]  # chgno is returned from "p4 review" so it must exist
                        ...
                        cux.execute(
                            ...
                            (
                                ...
                                dumps(
                                    self.trim_dict(
                                        cl,
                                        "chageType client user time change desc depotFile action rev job".split(),
                                    )
                                ),
                            ),
                        )
            ...
            for jobname in jobnames:
                ...
                cux.execute(
                    """insert or ignore into job (job, pickle) values (?, ?)""",
                    (jobname, dumps(self.trim_dict(job))),
                )
        ...
        if self.cfg.job_counter:
            ...
            try:
                ...
                if self.cfg.force:
                    ...
                    msg = (
                        """Job review counter ({jc}) is unset or invalid ({val}). """
            ...
            args = "{dfield}>{yr}/{mo}/{day}:{hr}:{min}:{sec}".format(
                dfield=self.cfg.job_datefield,
                yr=dt.year,
                mo=dt.month,
                day=dt.day,
                hr=dt.hour,
            ...
            for job in jobs:
                ...
                specs = [
                    ...
                    "//{}/job/{}*".format(
                        self.cfg.spec_depot, jobname
                    ),  # wildcard needed for suffixes
                ]
        ...
        self.db.commit()
        log.info(
            "{} change review(s).".format(
                self.db.execute("""select count(*) from rvw""").fetchone()[0]
            )
        )
        log.info(
            "{} job review(s).".format(
                self.db.execute("""select count(*) from jbrvw""").fetchone()[0]
            )
        )
    ...
    def change_summary(self, chgno):
        ...
        rv = self.db.execute(
            "select pickle from chg where chgno = ?", (chgno,)
        ).fetchall()
        assert len(rv) == 1
        cl = rv[0][0]
        clfiles = zip(
        ...
        # subject line
        subj = cfg.subject_template.format(
            **dict(p4port=self.cfg.p4port, chgno=chgno, desc=cl.get("desc"))
        )
        ...
        if jobs:
            ...
            jobs = ujobs
            jobsupdated = [
                self.txtwrpr_indented.fill(jb_tmpl.format(**job).strip())
                for job in jobs
            ]
            jobsupdated = "\n\n".join(jobsupdated)
        ...
        info = dict(
            ...
            dt=datetime.fromtimestamp(float(cl.get("time")))
            + timedelta(hours=self.cfg.timeoffset),
        ...
        if len(txt_summary) + len(html_summary) > self.cfg.max_email_size:
            html_info["jobsupdated"] = info["jobsupdated"] = "{} jobs...".format(
                len(jobs)
            )
    ...
    def job_summary(self, jobname):
        ...
        info["html_summary"] = self.cfg.html_job_template.format(
            jobdesc=html_summary, **info
        )
        return info
    ...
    def send_one_email_per_change(self):
        ...
        def email_chg_review(rvw):
            ...
            rv = self.db.execute(
                """select name, email from usr where usr = ?""", (author,)
            ).fetchall()
            if rv:
                aname, aemail = rv[0]
        ...
        if len(chgrvws) + len(jbrvws) > self.cfg.max_emails:
            log.fatal(
                "Will need to send {} emails, which exceed the limit of {}! Quitting.".format(
    ...
    def trim_dict(self, d, only=None):
        ...
        for k in d.keys():
            ...
            if val == None:
                ...
                if len(val) > maxlen:
                    val = val[:maxlen] + "...\n(truncated after {} characters)".format(
                        maxlen
                    )
            elif type(val) == type([]):
                newval = []
    ...
    def run(self):
        ...
        log.info(
            "Sent {} emails, took {}.".format(self.mail_sent, datetime.now() - dt1)
        )
        self.update_review_counters()
        self.cleanup()
        log.info(
            "Started {}, finished {}, took {}.".format(
                dt0, datetime.now(), datetime.now() - dt0
            )
        )
        return 0
...
if __name__ == "__main__":
    ...
    log.debug(
        "Running with Python version {}.{}.{} {} {}".format(
    ...
    if cfg.config_file and (cfg.p4passwd or cfg.smtp_passwd):
        ...
        if S_IRGRP & m or S_IWGRP & m or S_IROTH & m or S_IWOTH & m:
            log.warn(
                "You are storing plain text password(s) in the config file with insecure permission. Fix it!"
            )
    ...
    try:
        ...
        log.warn(
            "Using P4 CLI. Consider installing P4Python for better performance."
